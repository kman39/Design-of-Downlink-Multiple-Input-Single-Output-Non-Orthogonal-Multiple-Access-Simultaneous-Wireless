n = 100000;            %Epoch
Pt = BS.Pt;            %Transmit Power
first = -2;            %First (b=10^-2)
last  = 2;             %Last  (b=10^2)
step = 40;             %step of time slot
allb = 10.^(first:(last-first)/step:last);
...
function w = beamexp(unitw,idx,b)
    [M,K] = size(unitw);
    idx = exp(-idx*b);            %Power allocation exponential
    idx = repmat(idx,M,1);
    w = unitw.*idx;
end
